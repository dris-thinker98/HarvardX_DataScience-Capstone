...
source('C:/RStudio/ml_functions.R')
...
#Loading data from the data sets
...
The recommendation system will be created using all the tools learned throughout the courses in this series. I applied different dimensionality reduction algorithms: Neighborhood Approach and Matrix Factorization. It can be used to predit the rating of a user baed on an unrated movie. 
**RMSE** (Root-Mean-Squared-Error) has been applied as the evaluating criteria to analize the algorithm's performance.
...
#The RMSE function that will be used in this capstone project is:
...
The \textcolor{red}{`10 first rows`} of \textcolor{red}{`MovieLens dataset`} are:     
...
And, a more detailed information of \textcolor{red}{`MovieLens Dataset`} is:   
...
## Analysis based on Date (timestamp)   
...
Drama, Comedy, Action, and Thriller are the most likely rated. 
So which movies are the most rated?    
...
table.movies.rating <- table(group.rating %>% 
      ...
      'Count of Movies per Rating, with Different ID',
      'small')
...
## Analysis based on Rating & Year 
```{r table1.rating.year, echo=FALSE, include=FALSE}

# Analysis based on Rating

# <!-- Most/Least Rated years -->
...
Least rated year: \textcolor{red}{`r less.rated.year`}     
...
The graph of ratings per year is:   
...
## Analysis based on Rating & Movie
...
The least rated movie is: \textcolor{red}{`r less.rated.movie`}   
...
## Analysis based on Rating & Genre   
...
The least rated genre: \textcolor{red}{`r less.rated.genre`}   
...
The least rated title: \textcolor{red}{`r less.rated.title`}   
...
# <!-- Most Successful Movie Title per Year -->
...
The user with least ratings has the ID: \textcolor{red}{`r less.rated.user`}    
...
Graph of the number of times that a user has rated a movie:   
...
Here we can see that in the previous RMSEs,  \textcolor{red}{Movies Bias} and \textcolor{red}{Users Bias} are not the best option, but the \textcolor{red}{Users and Movies Bias} has the smallest RMSE.
...
Now, we will reduce the repeated movies, to one, in order to identify the mistakes in a better way. And, after joined the titles, the top Best Movies Ratings, are:       
...
# Identify the best lambda *from 0 to 7 at sequence of 0.25), over the movies bias
...
# Conclusion   
Machine Learning concepts and algorithms has been used to create a "Movie Recommendation System", and deduce that a join between "Users & Movies Bias", produce the best RMSE improvement for our System i.e., RMSE = `r min(RMSE.function.reg)`}
